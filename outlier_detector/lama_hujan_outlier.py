...
Q1 = df["lama_hujan"].quantile(q=0.25)
Q3 = df["lama_hujan"].quantile(q=0.75)
IQR: float = Q3 - Q1

lower_bound: float = Q1 - 1.5 * IQR
upper_bound: float = Q3 + 1.5 * IQR

outlier = (df["lama_hujan"] < lower_bound) | (df["lama_hujan"] > upper_bound)

y_jitter = np.random.normal(0, 0.02, size=len(df["lama_hujan"]))

plt.figure(figsize=(9, 3))
plt.scatter(df["lama_hujan"][~outlier], y_jitter[~outlier], label="Normal", alpha=0.7)
plt.scatter(df["lama_hujan"][outlier], y_jitter[outlier], label="Outlier", alpha=0.9)

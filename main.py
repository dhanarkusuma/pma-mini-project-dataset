...
class main : 
    ...
    def run(self):
        ...
        for scenario in scenarios:
            boost = ab.Adaboost(data)
            boost.initialize_parameter_tunning(param_tuning.get_param_grid(scenario))
            print(f"ada boost successfully trained for scenario {scenario} with parameter tuning using gridCV")
            self.evaluator = ev.Evaluator(boost.prediction_value(), data.get_y_test())
            # RMSE
            print(f"nilai rmse untuk ada boost : {self.evaluator.get_rmse()}")
            # MSE        
